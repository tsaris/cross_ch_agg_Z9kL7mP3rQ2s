...
class ClimaX(nn.Module):
    ...
    def forward_encoder(self, x: torch.Tensor, lead_times: torch.Tensor, variables):
        ...
        ###### CH PARALLEL ########
        x1 = x[:, 0:64, :, :]
        x1 = self.aggregate_variables_per_rank(x1)
        x1 = x1.unsqueeze(1)

        x2 = x[:, 64:128, :, :]
        x2 = self.aggregate_variables_per_rank(x2)
        x2 = x2.unsqueeze(1)

        x3 = x[:, 128:192, :, :]
        x3 = self.aggregate_variables_per_rank(x3)
        x3 = x3.unsqueeze(1)

        x4 = x[:, 192:256, :, :]
        x4 = self.aggregate_variables_per_rank(x4)
        x4 = x4.unsqueeze(1)
