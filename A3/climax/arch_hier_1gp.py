...
class ClimaX(nn.Module):
    ...
    def forward_encoder(self, x: torch.Tensor, lead_times: torch.Tensor, variables):
        ...
        variables_1 = variables[0:21]
        variables_2 = variables[21:42]
        variables_3 = variables[42:63]
        variables_4 = variables[63:84]
        ###########################
        
        if isinstance(variables, list):
            variables_1 = tuple(variables_1)
            variables_2 = tuple(variables_2)
            variables_3 = tuple(variables_3)
            variables_4 = tuple(variables_4)

        # tokenize each variable separately
        embeds_1, embeds_2, embeds_3, embeds_4 = [], [], [], []
        var_ids_1 = self.get_var_ids(variables_1, x.device)
        var_ids_2 = self.get_var_ids(variables_2, x.device)
        var_ids_3 = self.get_var_ids(variables_3, x.device)
        var_ids_4 = self.get_var_ids(variables_4, x.device)

        for i in range(len(var_ids_1)):
            id = var_ids_1[i]
            embeds_1.append(self.token_embeds[id](x[:, i : i + 1]))

        for i in range(len(var_ids_2)):
            id = var_ids_2[i]
            embeds_2.append(self.token_embeds[id](x[:, i : i + 1]))
            
        for i in range(len(var_ids_3)):
            id = var_ids_3[i]
            embeds_3.append(self.token_embeds[id](x[:, i : i + 1]))
            
        for i in range(len(var_ids_4)):
            id = var_ids_4[i]
            embeds_4.append(self.token_embeds[id](x[:, i : i + 1]))

        x_1 = torch.stack(embeds_1, dim=1)  # B, V, L, D
        x_2 = torch.stack(embeds_2, dim=1)  # B, V, L, D
        x_3 = torch.stack(embeds_3, dim=1)  # B, V, L, D
        x_4 = torch.stack(embeds_4, dim=1)  # B, V, L, D

        # add variable embedding
        var_embed_1 = self.get_var_emb(self.var_embed, variables_1)
        var_embed_2 = self.get_var_emb(self.var_embed, variables_2)
        var_embed_3 = self.get_var_emb(self.var_embed, variables_3)
        var_embed_4 = self.get_var_emb(self.var_embed, variables_4)
        
        x_1 = x_1 + var_embed_1.unsqueeze(2)  # B, V, L, D
        x_2 = x_2 + var_embed_2.unsqueeze(2)  # B, V, L, D
        x_3 = x_3 + var_embed_3.unsqueeze(2)  # B, V, L, D
        x_4 = x_4 + var_embed_4.unsqueeze(2)  # B, V, L, D

        ###### CH PARALLEL ########
        
        x_1 = self.aggregate_variables_per_rank(x_1)
        x_2 = self.aggregate_variables_per_rank(x_2)
        x_3 = self.aggregate_variables_per_rank(x_3)
        x_4 = self.aggregate_variables_per_rank(x_4)
